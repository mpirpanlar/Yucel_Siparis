...
begin
  if FTeklifId <= 0 then
  begin
    UniMainModule.saHata.Show('�nce teklifi kaydedin.');
    Exit;
  end;
...
begin
  ...
  begin
    qLoad.Close;
    UniMainModule.saHata.Show('Teklif veritaban�nda bulunamad�.');
    Exit;
  end;
  ...
  try
    ...
    if HasG then
      Sl.Add('<p><b>Ge�erlilik:</b> ' + HtmlEsc(FormatDateTime('dd/MM/yyyy HH:nn', GTar)) + '</p>');
    if SipNo <> '' then
      Sl.Add('<p><b>Sipari� No:</b> ' + HtmlEsc(SipNo) + '</p>');
    if Trim(Notlar) <> '' then
    begin
      Sl.Add('<p><b>A��klama</b></p><p>' + AciklamayiHtml(Notlar) + '</p>');
    end;
    Sl.Add('<table><thead><tr>');
    Sl.Add('<th>S�ra</th><th>Stok Kod</th><th>Stok Ad</th><th class="sayi">Miktar</th><th>Birim</th>');
    ...
    Sl.Add('</tbody></table>');
    Sl.Add('<p class="toplam">Sat�r Toplam� (Net): ' + FormatFloat('#,##0.00', TopNet) + '</p>');
    Sl.Add('</body></html>');
...
begin
  ...
  begin
    qLoad.Close;
    UniMainModule.saHata.Show('Teklif bulunamad�.');
    YeniKayit;
    Exit;
  end;
...
begin
  ...
  begin
    UniMainModule.saHata.Show('Ba�l�k zorunludur.');
    Exit;
  end;
  if VarIsNull(lkDurum.KeyValue) or VarIsEmpty(lkDurum.KeyValue) then
  begin
...
begin
  if FTeklifId <= 0 then
  begin
    UniMainModule.saHata.Show('�nce teklif ba�l���n� kaydedin.');
    Exit;
  end;
  ...
  if M <= 0 then
  begin
    UniMainModule.saHata.Show('Miktar s�f�rdan b�y�k olmal�d�r.');
    Exit;
  end;
  Tut := M * Bf;
