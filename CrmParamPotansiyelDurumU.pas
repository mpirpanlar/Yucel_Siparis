...
type
  TfrmCrmParamPotansiyelDurum = class(TUniForm)
    rootPanel: TUniPanel;
...
function frmCrmParamPotansiyelDurum: TfrmCrmParamPotansiyelDurum;
...
function frmCrmParamPotansiyelDurum: TfrmCrmParamPotansiyelDurum;
begin
  Result := TfrmCrmParamPotansiyelDurum(UniMainModule.GetFormInstance(TfrmCrmParamPotansiyelDurum));
end;

procedure TfrmCrmParamPotansiyelDurum.DetayYukle;
begin
  FYeni := False;
...
procedure TfrmCrmParamPotansiyelDurum.dsDurDataChange(Sender: TObject; Field: TField);
begin
  if not (csLoading in ComponentState) then
    DetayYukle;
end;

procedure TfrmCrmParamPotansiyelDurum.UniFormShow(Sender: TObject);
begin
  FYeni := False;
  btnListeleClick(Sender);
end;

procedure TfrmCrmParamPotansiyelDurum.btnListeleClick(Sender: TObject);
begin
  qDur.Close;
  qDur.SQL.Text :=
    'SELECT POTANSIYEL_DURUM_ID, KOD, ACIKLAMA, AKTIF, SIRA FROM dbo.CRM_POTANSIYEL_DURUM ORDER BY SIRA, POTANSIYEL_DURUM_ID';
  qDur.Open;
  DetayYukle;
end;

procedure TfrmCrmParamPotansiyelDurum.btnYeniClick(Sender: TObject);
begin
  FYeni := True;
  edKod.Text := '';
  edAciklama.Text := '';
...
procedure TfrmCrmParamPotansiyelDurum.btnKaydetClick(Sender: TObject);
var
  Sira: Integer;
  Did: Int64;
  YeniKod: string;
begin
  ...
  begin
    qExec.SQL.Text :=
      'INSERT INTO dbo.CRM_POTANSIYEL_DURUM (KOD, ACIKLAMA, AKTIF, SIRA) ' +
  ...
  begin
    ...
    Did := qDur.FieldByName('POTANSIYEL_DURUM_ID').AsLargeInt;
    qExec.SQL.Text :=
      'UPDATE dbo.CRM_POTANSIYEL_DURUM SET KOD = :KOD, ACIKLAMA = :ACIK, AKTIF = :AKT, SIRA = :SIRA ' +
      'WHERE POTANSIYEL_DURUM_ID = :DID';
...
procedure TfrmCrmParamPotansiyelDurum.btnKapatClick(Sender: TObject);
begin
