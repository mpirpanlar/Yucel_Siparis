...
type
  TfrmCrmParamAktiviteDurum = class(TUniForm)
    rootPanel: TUniPanel;
...
function frmCrmParamAktiviteDurum: TfrmCrmParamAktiviteDurum;
...
function frmCrmParamAktiviteDurum: TfrmCrmParamAktiviteDurum;
begin
  Result := TfrmCrmParamAktiviteDurum(UniMainModule.GetFormInstance(TfrmCrmParamAktiviteDurum));
end;

procedure TfrmCrmParamAktiviteDurum.DetayYukle;
begin
  FYeni := False;
...
procedure TfrmCrmParamAktiviteDurum.dsDurDataChange(Sender: TObject; Field: TField);
begin
  if not (csLoading in ComponentState) then
    DetayYukle;
end;

procedure TfrmCrmParamAktiviteDurum.UniFormShow(Sender: TObject);
begin
  FYeni := False;
  btnListeleClick(Sender);
end;

procedure TfrmCrmParamAktiviteDurum.btnListeleClick(Sender: TObject);
begin
  qDur.Close;
  qDur.SQL.Text :=
    'SELECT DURUM_ID, KOD, ACIKLAMA, AKTIF, SIRA FROM dbo.CRM_AKTIVITE_DURUM ORDER BY SIRA, DURUM_ID';
  qDur.Open;
  DetayYukle;
end;

procedure TfrmCrmParamAktiviteDurum.btnYeniClick(Sender: TObject);
begin
  FYeni := True;
  edKod.Text := '';
  edAciklama.Text := '';
...
procedure TfrmCrmParamAktiviteDurum.btnKaydetClick(Sender: TObject);
var
  Sira: Integer;
  Did: Int64;
  YeniKod: string;
begin
  ...
  begin
    qExec.SQL.Text :=
      'INSERT INTO dbo.CRM_AKTIVITE_DURUM (KOD, ACIKLAMA, AKTIF, SIRA) ' +
  ...
  begin
    ...
    Did := qDur.FieldByName('DURUM_ID').AsLargeInt;
    qExec.SQL.Text :=
      'UPDATE dbo.CRM_AKTIVITE_DURUM SET KOD = :KOD, ACIKLAMA = :ACIK, AKTIF = :AKT, SIRA = :SIRA ' +
      'WHERE DURUM_ID = :DID';
  ...
  begin
    Did := qDur.FieldByName('DURUM_ID').AsLargeInt;
    qExec.SQL.Text :=
      'UPDATE dbo.CRM_AKTIVITE SET DURUM = :KOD WHERE AKTIVITE_DURUM_ID = :DID';
    qExec.ParamByName('KOD').AsString := YeniKod;
    qExec.ParamByName('DID').AsLargeInt := Did;
    qExec.Execute;
  end;
...
procedure TfrmCrmParamAktiviteDurum.btnKapatClick(Sender: TObject);
begin
